<template>
  <div class='gamePage' >
    ...
    <v-layout wrap class='gameItem'>
      ...
      <v-flex xs12 lg8>
        <v-layout justify-end row wrap class='mt-4 mr-1'>
          <!-- Telegram -->
          <div class='mr-3'>
      ...
      <v-layout row wrap class='gameItem'>
        ...
        <v-flex xs12 lg8 >
              ...
              <carousel :scrollPerPage="true" :perPageCustom="[[10, 1], [1800, 1]]" :paginationSize=10 :paginationPadding=10 >
                <slide>
                  ...
                  <v-layout column wrap class='mt-4'>
                    ...
                    <!-- Last h2h -->
                    <v-layout row wrap class='stat_rank mt-2' v-for="(h2h, idx) in homeStatInfo.h2h" v-bind:key="idx" >
                        ...
                        <v-layout row wrap v-if='el.id' >
                          ...
                          <v-flex xs5 lg8 class='text-xs-left' v-bind:class="[{'pl-4': $vuetify.breakpoint.mdAndUp} ]">
                            <p class=''  style=''  >
                              ...
                              <a v-if='isCC' v-on:click='popUpYoutubeWindow(idx)' target='_blank' v-bind:title='el.shortTitle' class=' grey--text text--darken-3' style=''>{{el.shortTitle}}</a>
          ...
          <v-flex xs12 lg12>
            ...
            <!-- 베팅 history -->
            <div class='mt-3 mb-2' style='max-width:98%;' v-bind:class="{'mt-4 ml-4': $vuetify.breakpoint.mdAndUp}">
        ...
        <!-- 코멘츠 -->
        <v-flex xs12 lg4 class='mt-3' v-bind:class="[{'pr-5': $vuetify.breakpoint.lgAndUp } ]" >
          <v-flex xs12 class='mt-1 mb-2 text-xs-left'>
            <v-layout row wrap justify-space-between>
              <h3 class="betHistoryTxt ml-1 " style='line-height:30px;' >
                ...
                Comments
                <v-icon size=20 class='grey--text text--darken-1' v-on:click='_getComments(gameInfo.fixture_id,2)'>refresh</v-icon>
              </h3>
...
export default {
  ...
  methods: {
    ...
    async postReply2(idx, reIdx, replyId, mainCommentId, toNickname){ //부모 idx, 자식 replyId
      ...
      if(!replyText) return;
      // console.log('postReply2 : '+ idx, reIdx, replyId, mainCommentId, toNickname);
      ...
      try{
        ...
        // 이제 서버에서 DB에 저장만 제대로 하면된다.
        // 다음 delete처리한다.
        // 다음 읽어오는거 제대로 처리한다.
        // 그러면 게임내 게시판은 개발 완료다
        // DB에 저장
        const commentId = await GameAPI.reply2Comment(isMobile, replyId, toReply, this.userAccount, avatarInfo.avatarNum, mainCommentId, reIdx);

        // const commentId = await GameAPI.reply2Comment(isMobile, replyId, toReply, this.userAccount, avatarInfo.avatarNum, mainCommentId, );// 답글의 답글은 받아올 _id가 없으므로 그냥 서버로 던지면 된다
    ...
    async postComment(category) { // 1은 게임게시판
      ...
      // console.log(isMobile, this.gameInfo.fixture_id,  category, this.textareaTxt, this.userAccount);
      try{
        const avatarInfo = await this.getAvatarInfo(this.userAccount);

        // DB에 저장
        const commentId = await GameAPI.postComment(isMobile, this.gameInfo.fixture_id, this.gameInfo._id, category, this.textareaTxt, this.userAccount, avatarInfo.avatarNum);
        ...
        // 새로 추가된 betHistory 의 색을 2초간 분홍색으로 칠했다가 돌려놓기
        // 공지사항 때문에 newDom을 추가함. 돌이키고 싶으면 newDom 선언을 없애고 모두 다시 a로 바꾸면 됨.
        const a = document.querySelector('.newAppendedComment');
        const newDom = a.parentNode.children[1];
        if(newDom) {
    ...
    async _allHistoryFuc(fixtureId){
      ...
      allHistory.map( (info, idx)=>{
        allHistory[idx].avatarNum = allHistory.length - idx;
    ...
    async _web3Exe(){
      ...
      } catch(err){ console.log('There is no web.');}
    },
    ...
    replyComment2(idx, reIdx){// 부모 idx와 자식 reIdx
      // console.log(this.comments[idx].sub_comment_arr[reIdx]);
      // 아래는 event를 가져와 dom을 빼낸 후 클래스와 속성을 추가하고 제거하는 방법. 일단주석처리. 200130
      // const thisDom = event.target.parentNode.parentNode;
      // thisDom.classList.add('v-btn--disabled');
      // thisDom.setAttribute("disabled","disabled");
      // thisDom.classList.remove('v-btn--disabled');
      // thisDom.removeAttribute("disabled");
      // console.log(this.comments[idx].sub_comment_arr[reIdx].isWritingReply);

      this.comments[idx].sub_comment_arr.map( (c,i) => {
        if( i== reIdx) {c.isWritingReply = !c.isWritingReply; console.log('들어옴');}
