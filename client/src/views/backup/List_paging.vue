<template>
<!-- 이건 페이지 번호를 눌렀을때 그때마다 서버에서 가져오는 코드다. 개발을 완료했으나 sort()메모리 초과로 의미가 없어 다시 롤백하였다. 19.11.08 -->
  <div class='listPage'>
    <v-layout row wrap  >
      <v-flex xs12 lg2>
        ...
        <img v-else src='https://whitebetting.s3.amazonaws.com/football/logo/favicon.svg' alt="wB"  title="wB" class='mt-3' width='100' height='62' v-bind:class="[{'ml-5 ': $vuetify.breakpoint.lgAndUp }]"/>
      </v-flex>
      <v-flex xs12 lg4>
    ...
    <v-layout column v-if='page.category !=6'>
            ...
            <tr v-bind:active="props.selected" >
              ...
              <td class='gameListCell text-xs-center' >
                <router-link class='linkColor' v-bind:to='{path:`/football/${props.item.fixture_id}`}'>
                  <img v-if="props.index == 0" v-bind:src="`https://whitebetting.s3.amazonaws.com/allTeam/${props.item.homeTeam_id}.png`" v-on:error="apiFootballImg($event, props.item.homeTeam_id)" v-bind:alt="`${props.item.homeTeam}`" v-bind:title="`${props.item.homeTeam}`" class='teamLogo_sub'/>
              ...
              <td class="gameListCell text-xs-center" >
                <router-link class='linkColor' v-bind:to='{path:`/football/${props.item.fixture_id}`}'>
                  <img v-if="props.index == 0" v-bind:src="`https://whitebetting.s3.amazonaws.com/allTeam/${props.item.awayTeam_id}.png`" v-on:error="apiFootballImg($event, props.item.awayTeam_id)" v-bind:alt="`${props.item.awayTeam}`" v-bind:title="`${props.item.awayTeam}`" class='teamLogo_sub'/>
      ...
      <div class="text-xs-center pt-2 ">
        <v-pagination v-model="paginationSearched.page" v-bind:length="paginationSearched.len" color='blue lighten-2 ' v-bind:total-visible='paginationSearched.totalVisible' v-on:input="onPageChange"></v-pagination>
      </div>
    </v-layout>
    ...
    <v-layout column v-if='page.category == 6'>
      ...
      <v-flex v-if='page.category == 6' >
        <div class="text-xs-center pt-2 ">
          <!-- <v-pagination v-model="paginationSearched.page" :length="searchedPages" color='blue lighten-2 ' v-bind:total-visible='paginationSearched.totalVisible' v-on:click='aaa' ></v-pagination> -->
          <v-pagination v-model="paginationRankSearched.page" v-bind:length="paginationRankSearched.len" color='blue lighten-2 ' v-bind:total-visible='paginationRankSearched.totalVisible' v-on:input="onPageChange" ></v-pagination>
        </div>
      </v-flex >
    </v-layout>
...
export default {
  ...
  data () {
    return {
      ...
      paginationSearched : {page: 1, len:1, rowsPerPage : 10 , sortBy: 'number', totalVisible: 5 },
      paginationRankSearched : {page: 1, len:1, rowsPerPage : 10 , sortBy: 'number', totalVisible: 5 },
  ...
  methods : {
    ...
    onPageChange(pageNum){
      this._makePage( this.$route.params.name, pageNum );

    },
    async _makePage (pageName, pageNum=1) { // 페이지마다 다른 이름, 설명, 로고 넣어주기
      ...
      try{
        ...
        const g = await GameAPI.getGameList(this.page, pageNum );
        // if(!getGameList) this.$router.push('/'); // 나중에 해당 게임이 없을 때 보여줄 페이지
        const getGameList = g.gameList;
        this.paginationSearched.len = g.count;
        console.log('g.gameList: '+ g.gameList);
        // console.log(getGameList);
        if (category != 6){
          getGameList.map((gameInfo, idx)=>{
            gameInfo.event_date_moment = this.$moment(gameInfo.event_date).fromNow();
          } )
        } else { // user Rank 일때 리스트 정리
        // console.log(getGameList);
          getGameList.map( user =>{
  ...
  computed : {
    // 서버에서 페이지에 따라 정보를 가져오기 위해 주석처리 19.11.08
