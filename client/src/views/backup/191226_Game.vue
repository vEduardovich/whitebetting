<template>
  <div class='gamePage' >
    ...
    <v-layout wrap class='gameItem'>
      ...
      <v-flex xs4 lg3>
        <v-layout row wrap class='mt-4' >
          <!-- Telegram -->
          <div class='mr-3'>
            <a target="_blank" v-bind:href="sns.telegram" class="" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" title='share this match on telegram' ><v-icon class='fab fa-telegram-plane' style='color:#2196F3;'></v-icon></a>
          </div>

          <!-- twitter -->
          <div class='mr-3'>
            <a target="_blank" v-bind:href="sns.twitter" class="" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" title='share this match on twitter' ><v-icon class='fa fa-twitter' style='color:#2196F3;'></v-icon></a>
          </div>
          <!-- FaceBook -->
          <a title='share this match on facebook'>
    ...
    <v-layout wrap class='gameItem'>
      <!-- <v-flex lg5></v-flex> -->
      <v-flex xs12 lg3>
        <p class='teal--text text--lighten-2 scoreNotice'  v-if='gameStatusId == 4 ||  gameStatusId == 6'>The score is updated every 1 minute <v-icon size=20 class='grey--text text--darken-1 mr-1' v-on:click='_fetchData(gameInfo.fixture_id)'>refresh</v-icon> </p>
      </v-flex>
    </v-layout>
    ...
    <v-layout column wrap >
      <v-layout row wrap class='gameItem'>
                ...
                <div class="blue lighten-2 white--text pa-1 text-xs-center" >
                  <v-tooltip bottom :open-delay=0>
                    <template v-slot:activator="{ on }"  >
                      <a v-if='gameStatusId == 5 && gameInfo.reward_tx' v-on="on" class='mr-1 ' v-bind:href="'https://etherscan.io/tx/'+gameInfo.reward_tx" target='_blank'><v-icon class='	fas fa-external-link-alt external_link grey--text text--darken-3' style=""></v-icon></a>
                    </template>
        ...
        <!-- 경기 예측 -->
        <v-flex xs12 lg8>
          <v-layout column wrap class='my-4 mb-2' style='max-width:100%;' v-bind:class="[{'mt-4 ml-4': $vuetify.breakpoint.mdAndUp} ]" v-if="homeStatInfo && gameInfo.open_status == 3" >
            ...
            <carousel :scrollPerPage="true" :perPageCustom="[[10, 1], [1800, 1]]" :paginationSize=10 :paginationPadding=10 >
              <slide>            
                <v-layout column wrap >
                  ...
                  <!-- Rank row -->
                  <v-layout row wrap class='stat_rank ' >
                    <v-flex xs4  class=' text-xs-right  '>
                      <p >{{homeStatInfo.rank}}th</p>
                    </v-flex>
                ...
                <v-layout column wrap class='mt-4'>
                  ...
                  <!-- Last h2h -->
                  <v-layout row wrap class='stat_rank mt-2' v-for="(h2h, idx) in homeStatInfo.h2h" :key="idx" >
              ...
              <slide>
                <v-layout column wrap class='mt-2'  >
                  ...
                  <!-- Match Preview -->
                  <v-layout row wrap class='stat_rank mt-2' v-for="(el, idx) in media.guardian" :key="idx" >
                    <v-flex xs12 wrap class='text-xs-right ml-1 ' style='color:grey;margin-top:-5px;' v-bind:class="[{'text-xs-center': $vuetify.breakpoint.mdAndUp} ]">
                      <v-layout row wrap>
                        ...
                        <v-flex xs9 lg10 class='text-xs-left' v-bind:class="[{'pl-4': $vuetify.breakpoint.mdAndUp} ]">
                          <p class='mediaTxt '>
                            <a v-bind:href=el.webUrl target='_blank' v-bind:title=el.webUrl class='grey--text text--darken-3' style='text-decoration: underline; '>{{el.webTitle}}</a>
                          </p>
                        </v-flex>
                      </v-layout>
                ...
                <v-layout column wrap class='mt-3'>
                  <v-flex xs12 v-if=media.youtube >
                    <h3 class='stat_grey text-xs-center '>Videos</h3>
                  </v-flex>
                  <!-- Youtube Preview -->
                  <v-layout row wrap class=' mt-2' v-for="(el, idx) in media.youtube" :key="idx" >
                    <v-flex xs12 class='text-xs-right ml-1 mb-1' style='color:grey;margin-top:-5px;' v-bind:class="[{'text-xs-center': $vuetify.breakpoint.mdAndUp} ]">
                      <v-layout row wrap v-if='el.id.videoId' >
                        <v-flex xs3 lg2 class='pr-2'>
                          <span class='stat_vs' style=''>{{el.snippet.moment}}</span>
                        </v-flex>
                        <v-flex xs5 lg8 class='text-xs-left' v-bind:class="[{'pl-4': $vuetify.breakpoint.mdAndUp} ]">
                          <p class=''  style=''  >
                            
                            <a v-if='isCC' v-on:click='popUpYoutubeWindow(idx)' target='_blank' v-bind:title='el.snippet.shortTitle' class=' grey--text text--darken-3' style=''>{{el.snippet.shortTitle}}</a>
                            <!-- <a v-if='isCC' v-bind:href='`https://www.youtube.com/watch?v=${el.id.videoId}`' target='_blank' v-bind:title='el.snippet.shortTitle' class=' grey--text text--darken-3' style=''>{{el.snippet.shortTitle}}</a> -->
                            <!-- <a v-if='!isCC' v-bind:href='`https://www.youtube.com/watch?v=${el.id.videoId}`' target='_blank' v-bind:title='el.snippet.title' class=' grey--text text--darken-3' style=''>{{el.snippet.title}}</a>                         -->
                            <a v-if='!isCC' v-on:click='popUpYoutubeWindow(idx)' v-bind:title='el.snippet.title' class=' grey--text text--darken-3' style=''>{{el.snippet.title}}</a>                        
                          </p>
                        </v-flex>
                        <v-flex xs3 lg2>
                          <div class='stat_youtube' >
                            <a v-bind:href='`https://www.youtube.com/watch?v=${el.id.videoId}`' target='_blank' v-bind:title='el.snippet.title' class=' grey--text text--darken-3' style=''>                            
                              <img v-bind:src="`${el.snippet.thumbnails.default.url}`" alt='thumbnail' title='thumbnail' class=''  style='margin-top:-10px;'/>
                            </a>
                          </div>
                        </v-flex>
        ...
        <!-- 개인 베팅 통계 -->
        <v-flex xs12 lg8>
          <div class='mt-0 mb-2' style='max-width:100%;' v-bind:class="[{'mt-0 ml-4': $vuetify.breakpoint.mdAndUp} ]" >
            <v-layout align-end wrap>
                <span class="betHistoryTxt " v-bind:class="{'ml-1': $vuetify.breakpoint.mdAndDown}" >
                  <v-icon style='line-height:15px;font-size:18px;' class='fas fa-certificate' ></v-icon>
                  Betting statistics
...
export default {
  ...
  methods: {
    ...
    popUpYoutubeWindow(what){
      ...
      this.youtube.title = this.media.youtube[what].snippet.title;
      this.youtube.embeded = this.media.youtube[what].embeded;
      this.youtube.description = this.media.youtube[what].snippet.description;
    ...
    async _fetchYoutubeMatch(gameInfo){
      try{
        const mediaKey = 'AIzaSyANa2jGdwVXgWFldv42UY_c4Nxun_vVPgc';
        const q = `${gameInfo.homeTeam} v ${gameInfo.homeTeam}`
        const mediaQuery = `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=3&q=${q}&key=${mediaKey}`;

        const media = await GameAPI.youtubeMatch(mediaQuery); // 영상 정보
        media.items.map( el => {
          el.snippet.moment = this.$moment(el.snippet.publishedAt).fromNow();
          el.snippet.shortTitle = el.snippet.title.slice(0,50);
          el.embeded = `<iframe width="100%" height="auto" src="https://www.youtube.com/embed/${el.id.videoId}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
        });
        this.media.youtube = media.items;
        console.log(this.media.youtube);
    ...
    async _fetchStat(gameInfo, fixtureId){
      try{
        ...
        if(statInfo){
          const homeStatInfo = statInfo[0];
          const awayStatInfo = statInfo[1];
    ...
    async _fetchData(fixtureId){
      try{
        ...
        const fb_href = `https://whitebetting.com/game/${fixtureId}`;
...
@media screen and (min-width:0px) and (max-width: 960px){
  ...
  .stat_TeamName {white-space: nowrap;text-overflow: ellipsis;}
